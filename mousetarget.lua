...
function mod:MouseOverInfo(unit)
	...
	if isPlayer and db.showTalent and UnitIsConnected(unit) then
		if UnitLevel(unit) >= 10 then
			...
			mod:RegisterEvent("INSPECT_TALENT_READY");
			--save it
			if not Icetip_InspectTalent[name] then
				Icetip_InspectTalent[name] = {}
			end
			NotifyInspect(unit)
			--self:SetScript("OnEvent", Icetip_MouseTarget.INSPECT_TALENT_READY)
		end
	end
...
function mod:INSPECT_TALENT_READY()
	self:UnregisterEvent("INSPECT_TALENT_READY");
	local currTalentGroupId = GetActiveTalentGroup(true)

	local name1,_,point1 = GetTalentTabInfo(1,true, nil, currTalentGroupId)
	local name2,_,point2 = GetTalentTabInfo(2,true, nil, currTalentGroupId)
	local name3,_,point3 = GetTalentTabInfo(3,true, nil, currTalentGroupId)
	...
	local name1,_,point1 = GetTalentTabInfo(1,true, nil, secTalentGroupId);
	local name2,_,point2 = GetTalentTabInfo(2,true, nil, secTalentGroupId);
	local name3,_,point3 = GetTalentTabInfo(3,true, nil, secTalentGroupId);
	local pcolor1, pcolor2, pcolor3 = ColorTalent(point1), ColorTalent(point2),ColorTalent(point3);
